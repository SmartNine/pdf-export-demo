    ...
    if (imgObj.originalFile) {
      ...
      return new Promise((resolve) => {
        ...
        img.onload = () => {
          ...
          // 🔧 新增：检测图片是否可能是CMYK（通过颜色特征推断）
          const imageData = ctx.getImageData(
            0,
            0,
            Math.min(100, img.naturalWidth),
            Math.min(100, img.naturalHeight)
          );
          const cmykLikely = detectPossibleCMYK(imageData);

          if (cmykLikely) {
            console.warn("⚠️ 检测到可能的CMYK图片，建议后端处理");
          }

          canvas.toBlob(
            (blob) => {
              // 🔧 为blob添加元数据，告诉后端这需要CMYK处理
              blob.needsCMYKConversion = true;
              blob.originalColorSpace = "sRGB";
              console.log(
                `✅ 图片预处理完成（标记CMYK转换）: ${imgObj.originalFileName}`
              );
              resolve(blob);
            },
            "image/jpeg",
...
// 🆕 新增：简单的CMYK检测函数
function detectPossibleCMYK(imageData) {
  ...
  for (let i = 0; i < data.length; i += 4) {
    ...
    // CMYK图片在转换后可能出现的颜色特征
    const colorDeviation = Math.abs(r - g) + Math.abs(g - b) + Math.abs(r - b);
    if (colorDeviation > 100) {
      suspiciousPixels++;
    }
  }

  const suspiciousRatio = suspiciousPixels / (data.length / 4);
...
async function exportMultipleRegions() {
  ...
  try {
    ...
    for (const regionId of uvRegionIds) {
      ...
      const svgStyles = generateSVGStyles(usedFontNames, fontUrlMap, "cmyk");
...
async function sendMultiRegionExportRequest(regionExports) {
  ...
  // 🔧 新增：颜色管理配置
  formData.append("colorProfile", "cmyk");
  formData.append("iccProfile", "Japan Color 2001 Coated");
  formData.append("printReady", "true");
  formData.append("colorIntent", "perceptual");
  ...
  for (const imgObj of images) {
    if (allImageFileNames.includes(imgObj.originalFileName)) {
      ...
      // 🔧 新增：为每个图片添加颜色处理指令
      formData.append(
        `imageColorHint_${imgObj.originalFileName}`,
        "needsCMYKConversion"
      );
    }
  }
  ...
  if (result.success) {
    ...
    // 🔧 增强成功消息，显示颜色管理信息
    const colorInfo = result.usedICC
      ? "使用Japan Color 2001配置"
      : "标准CMYK转换";
...
async function exportDesign() {
  ...
  try {
    ...
    const fontUrlMap = new Map(fontOptions.map((f) => [f.name, f.url]));
    const svgStyles = generateFontStylesForSVG(usedFontNames, fontUrlMap);
...
function generateSVGStyles(fontNames, fontUrlMap, colorProfile = "cmyk") {
  let styles = "";
  ...
  if (colorProfile === "cmyk") {
    styles += `
      ...
      /* Japan Color 2001 颜色配置提示 */
      svg {
        color-profile: "Japan Color 2001 Coated";
      }
    `;
  }
...
async function saveLocally() {
  ...
  try {
    ...
    const usedFontNames = getUsedFonts(clonedCanvas);
    const fontUrlMap = new Map(fontOptions.map((f) => [f.name, f.url]));
    const fontStyles = generateSVGStyles(usedFontNames, fontUrlMap, "cmyk");
